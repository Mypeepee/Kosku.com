// app/Lelang/[slug]/[agentId]/components/AgentSidebarLelang.tsx
"use client";
...
function SurveyModal({ isOpen, onClose, agentName, propertyTitle, agentPhone, idProperty, idAgent }: SurveyModalProps) {
  ...
  const handleClose = useCallback(() => { setVisible(false); setTimeout(onClose, 420); }, [onClose]);

  const handlePhoneInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    let v = e.target.value.replace(/[-\s]/g,"").replace(/\D/g,"");
    if (v.startsWith("62")) v = v.slice(2);
    if (v.startsWith("0"))  v = v.slice(1);
    setPhoneDigits(v.slice(0,12)); setFormError("");
  };
  ...
  const subtitles: Record<number,string> = { 1:propertyTitle, 2:fmtDate(), 3:`${fmtDate()} · ${selectedTime} WIB`, 4:`${fmtDate()} · ${selectedTime} WIB` };

  const BottomRow = ({ onNext, nextLabel, nextDisabled=false }: { onNext:()=>void; nextLabel:string; nextDisabled?:boolean }) => (
    <div className="flex gap-2 pt-2">
      <button onClick={goBack}
        className="flex-1 py-3.5 rounded-2xl font-bold text-sm transition-all flex items-center justify-center gap-1.5 text-white/40 hover:text-white/80"
        style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)" }}>
        <Icon icon="solar:alt-arrow-left-bold" className="text-[11px]"/>Kembali
      </button>
      <button onClick={onNext} disabled={nextDisabled}
        className="flex-[2.2] py-3.5 rounded-2xl font-extrabold text-sm transition-all flex items-center justify-center gap-2 disabled:opacity-20 disabled:cursor-not-allowed active:scale-[0.98]"
        style={!nextDisabled?{background:"#ffffff",color:"#000000"}:{background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.2)"}}>
        {nextLabel}<Icon icon="solar:alt-arrow-right-bold" className="text-[11px]"/>
      </button>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center sm:px-4">
      ...
      <div className="relative w-full sm:max-w-[440px] sm:mx-auto rounded-t-[2.25rem] sm:rounded-[2rem] overflow-hidden max-h-[92dvh] sm:max-h-[85vh] flex flex-col"
        ...
        <div className="px-5 pt-4 pb-4">
          <div className="flex items-center gap-1.5 mb-3.5">
            {Array.from({length:4}).map((_,i) => (
              <div key={i} className="h-[3px] rounded-full" style={{ width:step===i+1?28:16, background:step===i+1?"#86efac":step>i+1?"rgba(134,239,172,0.35)":"rgba(255,255,255,0.1)", transition:"all 0.4s cubic-bezier(0.22,1.2,0.36,1)" }}/>
            ))}
            <span className="ml-auto text-[10px] font-bold text-white/20 tabular-nums">{step}/4</span>
          </div>
          <div className="flex items-start justify-between gap-3">
            ...
            <button onClick={handleClose}
              className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all hover:bg-white/10 mt-0.5"
        ...
        {step===1 && (
          <div className="px-5 py-4">
            <div className="flex items-center justify-between mb-4">
              <button onClick={()=>navMonth(-1)} disabled={atMin} className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:bg-white/10 disabled:opacity-20" style={{background:"rgba(255,255,255,0.05)"}}>
              ...
              <button onClick={()=>navMonth(1)} className="w-8 h-8 rounded-xl flex items-center justify-center transition-all hover:bg-white/10" style={{background:"rgba(255,255,255,0.05)"}}>
              ...
              {Array.from({length:daysInMonth}).map((_,i)=>{
                ...
                return (
                  <button key={d} onClick={()=>!past&&goToStep2(new Date(yr,mo,d))} disabled={past}
                    className={`relative h-9 rounded-xl text-sm font-bold transition-all duration-150 ${sel?"bg-[#86efac] text-black shadow-[0_0_18px_rgba(134,239,172,0.35)]":past?"cursor-not-allowed":tod?"text-[#86efac] ring-1 ring-[#86efac]/35 hover:bg-[#86efac]/10":"hover:text-white hover:bg-white/[0.07]"}`}
        ...
        {step===2 && (
          <div className="px-5 py-4">
            <div className="flex items-center gap-2.5 rounded-2xl px-3.5 py-2.5 mb-4" style={{background:"rgba(134,239,172,0.06)",border:"1px solid rgba(134,239,172,0.12)"}}>
            ...
            <p className="text-[9px] font-black text-white/20 uppercase tracking-widest mb-2.5">{slotsLoading?"Memuat jadwal agent...":"Pilih Slot Waktu"}</p>
            <div className="grid grid-cols-3 gap-2">
              {slotsLoading
                ? Array.from({length:9}).map((_,i)=><div key={i} className="h-[52px] rounded-xl animate-pulse" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.05)"}}/>)
                : (() => {
        ...
        {step===3 && (
          <div className="px-5 py-4 space-y-3.5">
            <div className="flex items-center gap-2.5 rounded-2xl px-3.5 py-2.5" style={{background:"rgba(134,239,172,0.06)",border:"1px solid rgba(134,239,172,0.12)"}}>
            ...
            <div>
              <label className="text-[9px] font-black text-white/25 uppercase tracking-widest block mb-1.5">Nama Lengkap <span className="text-rose-400/70">*</span></label>
              <input type="text" value={namaKlien} onChange={e=>{setNamaKlien(e.target.value);setFormError("");}}
            ...
            <div>
              <label className="text-[9px] font-black text-white/25 uppercase tracking-widest block mb-1.5">Nomor Telepon <span className="text-rose-400/70">*</span></label>
              <div className="flex items-stretch rounded-2xl overflow-hidden transition-colors"
                ...
                <div className="flex items-center px-4 shrink-0 select-none" style={{background:"rgba(134,239,172,0.08)",borderRight:"1px solid rgba(255,255,255,0.08)"}}>
                  <span className="text-[13px] font-extrabold text-[#86efac] tracking-wide">+62</span>
                </div>
                <input type="tel" inputMode="numeric"
                  value={fmtPhoneDigits(phoneDigits)} onChange={handlePhoneInput}
            ...
            <div>
              <label className="text-[9px] font-black text-white/25 uppercase tracking-widest block mb-1.5">Catatan <span className="text-white/15 normal-case font-normal tracking-normal">(opsional)</span></label>
              <textarea value={catatan} onChange={e=>setCatatan(e.target.value)} placeholder="Tulis catatan untuk agent..." rows={2}
                className="w-full rounded-2xl px-4 py-3 text-white text-[13px] font-medium outline-none placeholder:text-white/15 resize-none transition-colors"
                style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)"}}
                onFocus={e=>e.currentTarget.style.borderColor="rgba(134,239,172,0.3)"}
                onBlur={e=>e.currentTarget.style.borderColor="rgba(255,255,255,0.08)"}/>
            </div>
          ...
          <div className="px-5 py-4">
            <div className="space-y-2 mb-4">
              {[
                ...
                <div key={item.label} className="flex items-center gap-3 rounded-2xl px-3.5 py-2.5" style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.05)"}}>
                  <div className="w-7 h-7 rounded-xl flex items-center justify-center shrink-0" style={{background:"rgba(134,239,172,0.1)"}}>
            ...
            <p className="text-center text-[10px] text-white/20 mb-3 font-medium">Jadwal dicatat otomatis &amp; konfirmasi dikirim via WhatsApp</p>
            <div className="flex gap-2">
              <button onClick={goBack} disabled={submitting}
                ...
                style={{background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.07)"}}>
                <Icon icon="solar:alt-arrow-left-bold" className="text-[11px]"/>Ubah
              </button>
              <button onClick={handleBook} disabled={submitting}
                ...
                {submitting?<div className="w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin"/>:<Icon icon="ic:baseline-whatsapp" className="text-lg"/>}
...
function OfferModal({ isOpen, onClose, agentName, propertyTitle, agentPhone, askingPrice, idProperty, idAgent, onOfferChange }: OfferModalProps) {
  ...
  const diffPct = offerNum > 0 && askingPrice > 0 ? Math.round(((askingPrice - offerNum) / askingPrice) * 100) : 0;
  const offerFeedback = offerNum > 0 ? getOfferFeedback(diffPct, "harga limit") : null;

  const handleSubmit = async () => {
    if (!offerRaw || offerNum < 1_000_000) { setError("Tawaran minimal Rp 1.000.000"); return; }
    if (!paymentMethod) { setError("Pilih cara bayar terlebih dahulu"); return; }
    ...
    let msg =
      ...
      `💰 Harga Limit: ${fmt(askingPrice)}\n🤝 Tawaran Saya: *${fmt(offerNum)}*`;
    if (diffPct > 0) msg += ` (${diffPct}% di bawah harga limit)`;
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center">
      <div onClick={handleClose} className="absolute inset-0 bg-black/70 backdrop-blur-md" style={{ opacity: visible ? 1 : 0, transition: "opacity 0.25s" }} />
      <div className="relative w-full sm:max-w-[420px] sm:mx-auto bg-[#0C0C0C] border border-white/[0.07] rounded-t-[2.5rem] sm:rounded-[2rem] overflow-hidden shadow-[0_-24px_80px_rgba(0,0,0,0.8)] transition-all duration-300 ease-out max-h-[92dvh] sm:max-h-[85vh] flex flex-col"
        style={{ transform: visible ? "translateY(0)" : "translateY(24px)", opacity: visible ? 1 : 0 }}>
        <div className="sm:hidden flex justify-center pt-3.5 pb-1 shrink-0"><div className="w-8 h-[3px] rounded-full bg-white/15" /></div>
        <div className="px-6 pt-5 pb-4 border-b border-white/[0.06] shrink-0">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] font-black text-[#86efac]/60 uppercase tracking-widest">Negosiasi Harga</span>
          ...
          <h2 className="text-[1.25rem] font-extrabold text-white leading-tight tracking-tight">Ajukan Penawaran</h2>
          ...
          {justAuthenticated ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between bg-white/[0.03] border border-white/[0.06] rounded-2xl px-4 py-3">
                <span className="text-[11px] text-white/40 font-medium">Harga Limit</span>
              ...
              <div>
                <label className="text-[9px] font-black text-white/30 uppercase tracking-widest block mb-2">Tawaran Anda</label>
                <div className={`flex items-center gap-2 rounded-2xl px-4 py-3.5 border transition-colors ${error?"border-rose-500/40 bg-rose-500/[0.04]":"border-white/[0.08] bg-white/[0.04] focus-within:border-[#86efac]/30"}`}>
                  <span className="text-sm font-bold text-white/30 shrink-0">Rp</span>
                  <input type="text" inputMode="numeric" value={displayVal} onChange={e=>{setOfferRaw(e.target.value.replace(/\D/g,""));setError("");}}
                    placeholder="0" className="flex-1 bg-transparent text-white font-extrabold text-lg outline-none placeholder:text-white/15 tabular-nums"/>
                </div>
                {error ? (
              ...
              <div>
                ...
                <textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Contoh: Siap KPR, bisa survey minggu ini..." rows={2}
              ...
              <p className="text-center text-[10px] text-white/20 font-medium">Penawaran dikirim langsung ke agent untuk dipertimbangkan</p>
            </>
          )}
        </div>
      </div>
...
export default function AgentSidebar({ data, currentAgentId, presentingAgent = null, onShareOpen }: AgentSidebarProps) {
  ...
  const askingPrice: number = data.priceRates?.monthly || data.harga || 0;
  ...
  const isCobrokeViewer = !!viewerAgentId && viewerAgentId !== String(agentData.id);
    ...
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <div className="w-full h-full rounded-full overflow-hidden" style={{ padding: 2, background: "linear-gradient(135deg, #86efac 0%, #34d399 100%)" }}>
        <div className="w-full h-full rounded-full overflow-hidden relative bg-[#1A1A1A] flex items-center justify-center">
          {hasAvatar ? (
            <Image src={avatarSrc} alt={agentData.name} fill className="object-cover" sizes={`${size}px`} />
          ) : (
  ...
  return (
    <>
      <SurveyModal
        ...
        agentName={agentData.name} propertyTitle={data.title || data.judul || ""}
        agentPhone={agentData.phone} idProperty={data.id_property}
        idAgent={String(agentData.id)}
      />
      <OfferModal
        isOpen={offerOpen} onClose={() => setOfferOpen(false)}
        agentName={agentData.name} propertyTitle={data.title || data.judul || ""}
        agentPhone={agentData.phone} askingPrice={askingPrice}
        idProperty={data.id_property} idAgent={agentData.id}
        onOfferChange={setMyOffer}
      />
      <CobrokeModal
        isOpen={cobrokeOpen} onClose={() => setCobrokeOpen(false)}
        agentName={agentData.name} agentOffice={agentData.office} agentPhone={agentData.phone}
        propertyTitle={data.title || data.judul || ""} askingPrice={askingPrice}
        idProperty={data.id_property} idAgent={agentData.id}
      ...
      <div className="hidden lg:flex flex-col w-[360px] shrink-0 self-start sticky top-24 max-h-[calc(100dvh-7rem)] overflow-y-auto no-scrollbar rounded-[1.75rem]"
        ...
        <div className="px-6 pt-6 pb-5">
          <span className="text-[10px] font-black text-[#86efac]/70 uppercase tracking-[0.18em] block mb-2">Harga Limit</span>
          <p className="text-[2rem] font-black text-white leading-none tracking-tight">{fmt(askingPrice)}</p>
          <div className="flex items-center gap-1.5 mt-2.5">
            <div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse" />
            <span className="text-[10px] font-bold text-white/35 uppercase tracking-widest">Tersedia</span>
          </div>
        </div>
        ...
        <div className="px-6 py-5">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h4 className="text-white font-extrabold text-[0.95rem] truncate leading-tight">{agentData.name}</h4>
              <p className="text-[11px] text-white/35 truncate mb-2">{agentData.office}</p>
        ...
        <div className="grid grid-cols-3">
          {[
            { icon:"solar:star-bold",     iconCls:"text-amber-400", val:agentData.rating.toFixed(1), label:"Rating" },
            { icon:"solar:bolt-circle-bold", iconCls:"text-[#86efac]", val:"Fast",                 label:"Respon" },
            { icon:"solar:map-point-bold", iconCls:"text-rose-400",  val:agentData.area,           label:"Area"   },
          ].map((s,i) => (
        ...
        <div className="px-6 py-4 flex items-center gap-3">
          ...
          <p className="text-[10px] text-white/30 leading-snug font-medium">Hati-hati penipuan. Jangan transfer ke rekening pribadi agent.</p>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50"
        style={{ background:"#0C0C0C", borderTop:"1px solid rgba(255,255,255,0.07)", boxShadow:"0 -24px 60px rgba(0,0,0,0.85)" }}>
        <div className="px-4 pt-3 pb-3">
          {/* Row 1: Agent + price */}
          <div className="flex items-center gap-3 mb-3">
            <Avatar size={38} />
            <div className="flex-1 min-w-0">
              <p className="text-white font-bold text-[13px] truncate leading-tight">{agentData.name}</p>
              <div className="flex items-center gap-1.5 mt-[3px]">
                <Icon icon="solar:star-bold" className="text-amber-400 text-[9px]" />
                <span className="text-[10px] font-extrabold text-white/55">{agentData.rating.toFixed(1)}</span>
                <span className="text-white/20 text-[10px]">·</span>
                <span className="text-[10px] text-white/35 truncate">{agentData.area}</span>
              </div>
            </div>
            <div className="shrink-0 text-right">
              <p className="text-[8px] font-bold text-white/30 uppercase tracking-widest leading-none">Harga Limit</p>
              <p className="text-[1.05rem] font-black text-white leading-tight whitespace-nowrap tabular-nums">
                Rp {fmtShort(askingPrice)}
              </p>
            </div>
          </div>
              ...
              <>
                {myCobroke?.status === "pending" ? (
                  <button onClick={() => setCobrokeOpen(true)}
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest relative z-10 text-shimmer-amber">Menunggu</span>
                  </button>
                ) : myCobroke?.status === "diterima" ? (
                  <button onClick={() => setCobrokeOpen(true)}
                    ...
                    <span className="text-[8px] font-black text-[#86efac] uppercase tracking-widest">Co-Broke Aktif</span>
                  </button>
                ) : (
                  <button onClick={() => setCobrokeOpen(true)}
                    ...
                    <span className="text-[8px] font-black uppercase tracking-widest relative z-10">Co-Broke</span>
                  </button>
                )}
                <button onClick={handleDownloadBrosur} disabled={isDownloadingBrosur}
                  className="flex-1 flex flex-col items-center justify-center gap-[3px] rounded-2xl transition-all active:scale-[0.96] disabled:opacity-60"
                  style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.09)" }}>
                  <Icon icon={isDownloadingBrosur ? "solar:refresh-bold" : "solar:gallery-download-bold-duotone"} className={`text-white/70 text-[17px] ${isDownloadingBrosur ? "animate-spin" : ""}`} />
                  <span className="text-[8px] font-black text-white/35 uppercase tracking-widest">Brosur</span>
                </button>
              </>
            ) : (
              <>
                {myOffer?.status === "pending" ? (
                  ...
                  <button onClick={() => setOfferOpen(true)}
                    ...
                    <span className="text-[8px] font-black text-white/35 uppercase tracking-widest">Penawaran</span>
                  </button>
                )}
                <button onClick={() => setSurveyOpen(true)}
                  className="flex-1 flex flex-col items-center justify-center gap-[3px] rounded-2xl transition-all active:scale-[0.96]"
                  ...
                  <span className="text-[8px] font-black text-white/35 uppercase tracking-widest">Survei</span>
                </button>
              </>
            )}
            </>

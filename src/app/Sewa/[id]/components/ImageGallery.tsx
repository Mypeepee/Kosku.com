 ...
 * terbaca sebagai kos yang malas didokumentasikan. Yang dilakukan di sini,
...
export default function ImageGallery({ images, judul, tersewa }: Props) {
  const foto = images.length > 0 ? images : [PLACEHOLDER];
        ...
        <div className="grid grid-cols-4 grid-rows-2 gap-2.5 h-[460px] rounded-[1.75rem] overflow-hidden">
          {grid.map((src, i) => (
            <button
              key={`${src}-${i}`}
              onClick={() => buka(i)}
              className={`group relative overflow-hidden ${tataLetak[i]}`}
          ...
          {foto.map((src, i) => (
            ...
            >
              <Image
                ...
                priority={i === 0}
                sizes="100vw"
                className={`object-cover ${tersewa ? "grayscale" : ""}`}
              />
            </div>
          ))}
        </div>
      ...
      {mounted &&
        ...
        createPortal(
          <div
            ...
            style={{
              background:
                "radial-gradient(120% 90% at 50% 0%, #121826 0%, #080B12 55%, #04060A 100%)",
              opacity: tampil ? 1 : 0,
              ...
              {!dimuat[indeks] && (
                <div className="pointer-events-none absolute inset-0 grid place-items-center">
                  <div className="flex flex-col items-center gap-3">
                    <span className="h-8 w-8 animate-spin rounded-full border-2 border-white/15 border-t-sky-400" />
                    ...
                    {foto.map((src, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          {aktif && (
                            <span className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 rounded-b-lg bg-gradient-to-t from-emerald-400/25 to-transparent sm:rounded-b-xl" />
                          )}
                        </button>
                      );
                    })}

...
export default function AgentSidebar({ data }: AgentSidebarProps) {
  ...
  // 1. DATA MAPPING
  const agentData = {
    name: data.owner.name,        // Dari tabel pengguna.nama_lengkap
    avatar: data.owner.avatar,    // Dari tabel pengguna.foto_profil_url
    phone: data.owner.phone || "628123456789",
    office: data.owner.office,    // Dari tabel agent.nama_kantor
    rating: data.owner.rating,    // Dari tabel agent.rating
    area: data.owner.area,        // Dari tabel agent.kota_area (PENGGANTI TERJUAL)
  };
  ...
  const handleContact = (type: "wa" | "call" | "schedule") => {
    ...
    if (type === "wa") {
        ...
        const text = encodeURIComponent(`Halo ${agentData.name}, saya inginmenjadwalkan *Survei Lokasi* untuk properti: *${data.title}*.`);
        window.open(`https://wa.me/${phone}?text=${text}`, "_blank");
    }
  };
      ...
      <div className="flex-1 overflow-y-auto px-6 py-2 custom-scrollbar space-y-4">
         ...
         <div className="bg-[#0F0F0F] border border-white/10 rounded-xl p-4 flex items-center gap-4">
             ...
             <div className="flex-1 min-w-0">
                 {/* NAMA LENGKAP (Dari Tabel Pengguna) */}
         ...
         <div className="flex justify-between items-start bg-[#0F0F0F] border border-white/10 rounded-xl p-3 px-2">
            ...
            {/* Kolom 3: AREA (FIXED: Full Width, No Truncate) */}
            <div className="flex flex-col items-center justify-center gap-1 flex-[1.2] px-1">
                <div className="flex items-center justify-center gap-1 w-full text-center">
                    ...
                    {/* HAPUS TRUNCATE & MAX-W, BIARKAN MENGENAI REL */}
